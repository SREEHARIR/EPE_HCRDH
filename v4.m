...
%             continue;
        ...
        elseif ii ~= 1 && jj == 1
            ...
            delta = min(abs(I(ii,jj)-In(ii,jj-1)),abs(I(ii,jj)-In(ii-1,jj)));
            deltainv = min(abs(Iinv(ii,jj)-In(ii,jj-1)),abs(Iinv(ii,jj)-In(ii-1,jj)));
        end
        if(delta > deltainv)
            In(ii,jj) = Iinv(ii,jj);
%             In(ii,jj) = I(ii,jj);
...
%             continue;
        if ii == 1 && jj == 1
            ...
            deltainv = 1;
        elseif ii == 1 && jj ~= 1
            delta = abs(In(ii,jj)-In(ii,jj-1));
            deltainv = abs(Iinv(ii,jj)-In(ii,jj-1));
        elseif ii ~= 1 && jj == 1
            delta = abs(In(ii,jj)-In(ii-1,jj));
            deltainv = abs(Iinv(ii,jj)-In(ii-1,jj));
        else
            delta = min(abs(In(ii,jj)-In(ii,jj-1)),abs(In(ii,jj)-In(ii-1,jj)));
            deltainv = min(abs(Iinv(ii,jj)-In(ii,jj-1)),abs(Iinv(ii,jj)-In(ii-1,jj)));
        end
        if(delta > deltainv)
            fprintf('[%i %i]error occured\n',ii,jj);
        elseif delta == deltainv
            fprintf('[%i %i]equal occured\n',ii,jj);
        else
...
for ii = 1:512
    for jj  = 1:512
        ...
        elseif ii ~= 1 && jj == 1
            ...
            delta0 = min(abs(Id0m-Id(ii,jj-1)),abs(Id0m-Id(ii-1,jj)));
            delta1 = min(abs(Id1m-Id(ii,jj-1)),abs(Id1m-Id(ii-1,jj)));
        end
        if delta0 < delta1
